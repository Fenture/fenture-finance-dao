module ffd_admin::fenture_finance_dao {
    ...
    public entry fun initialize(coin_admin: &signer) {
        assert!(signer::address_of(coin_admin) == @ffd_admin, error::permission_denied(ENOT_ADMIN));
        ...
        move_to(coin_admin, CapStore<BurnCapability<FentureFinanceDao>> { cap: burn_cap });
        move_to(coin_admin, CapStore<FreezeCapability<FentureFinanceDao>> { cap: freeze_cap });
        move_to(coin_admin, CapStore<MintCapability<FentureFinanceDao>> { cap: mint_cap });
        move_to(coin_admin, Delegations<BurnCapability<FentureFinanceDao>>{ inner: vector::empty<DelegatedCapability<BurnCapability<FentureFinanceDao>>>() });
        move_to(coin_admin, Delegations<FreezeCapability<FentureFinanceDao>>{ inner: vector::empty<DelegatedCapability<FreezeCapability<FentureFinanceDao>>>() });
        move_to(coin_admin, Delegations<MintCapability<FentureFinanceDao>>{ inner: vector::empty<DelegatedCapability<MintCapability<FentureFinanceDao>>>() });
        move_to(coin_admin, AdminStore{ admin: @ffd_admin });
    ...
    public entry fun change_admin(admin: &signer, new_admin: address) acquires AdminStore {
        only_admin(admin);
        borrow_global_mut<AdminStore>(@ffd_admin).admin = new_admin;
    }

    public entry fun delegate_capability<CapType>(admin: &signer, to: address) acquires AdminStore, Delegations {
        only_admin(admin);
        let delegations = &mut borrow_global_mut<Delegations<CapType>>(@ffd_admin).inner;
        let i = 0;
    ...
    public entry fun claim_capability<CapType: copy + store>(account: &signer) acquires Delegations, CapStore {
        ...
        let cap = borrow_global<CapStore<CapType>>(@ffd_admin).cap;
    ...
    fun is_admin(admin: &signer): bool acquires AdminStore {
        signer::address_of(admin) == borrow_global<AdminStore>(@ffd_admin).admin
    }
